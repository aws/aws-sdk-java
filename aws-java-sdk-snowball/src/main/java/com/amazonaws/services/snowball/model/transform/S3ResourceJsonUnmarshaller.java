 ...
 */
public class S3ResourceJsonUnmarshaller implements
        Unmarshaller<S3Resource, JsonUnmarshallerContext> {
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("BucketArn", targetDepth)) {
                    context.nextToken();
                    s3Resource.setBucketArn(context.getUnmarshaller(
                            String.class).unmarshall(context));
                }
                if (context.testExpression("KeyRange", targetDepth)) {
                    context.nextToken();
                    s3Resource.setKeyRange(KeyRangeJsonUnmarshaller
