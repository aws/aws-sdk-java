...
public class GetHealthCheckStatusRequest extends AmazonWebServiceRequest implements Serializable, Cloneable {
     ...
     * the <code>HealthCheckId</code> element and specify the ID of the
     * applicable health check. <p>Amazon Route 53 determines whether a
     * resource record set is healthy by periodically sending a request to
     * the endpoint that is specified in the health check. If that endpoint
     ...
     * record set unhealthy. <p>The <code>HealthCheckId</code> element is
     ...
     * configurations: <ul> <li>You're checking the health of the resource
     * record sets in a weighted, latency, geolocation, or failover resource
     * record set, and you specify health check IDs for all of the resource
     * record sets. If the health check for one resource record set specifies
     * an endpoint that is not healthy, Amazon Route 53 stops responding to
     * queries using the value for that resource record set.</li> <li><p>You
     * set <code>EvaluateTargetHealth</code> to <code>true</code> for the
     ...
     * about this configuration, see <a>EvaluateTargetHealth</a>. <p>Amazon
     ...
     * <code>HealthCheckId</code> element to a resource record set, Amazon
     * Route 53 checks the health of the endpoint that you specified in the
     * health check. </li> </ul> <p>For geolocation resource record sets, if
     ...
     * value of CountryCode is <code>*</code>), in that order, until it finds
     * a resource record set for which the endpoint is healthy. <p>If your
     ...
     * server (such as <code>us-east-1-www.example.com</code>), not the name
     * of the resource record sets (example.com). <important>In this
     ...
     * record sets, health check results will be unpredictable.</important>
     * <p>
     * <b>Constraints:</b><br/>
     * <b>Length: </b>0 - 64<br/>
     */
