...
class ResourceRecordSetsChanged {
    ...
    static class IsINSYNCMatcher extends WaiterAcceptor<GetChangeResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"INSYNC\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }

        private static final JmesPathExpression ast = new JmesPathSubExpression(new JmesPathField("ChangeInfo"), new JmesPathField("Status"));
         ...
         */
        @Override
        public boolean matches(GetChangeResult result) {
