...
public class EventMarshaller {
    ...
    private static final MarshallingInfo<String> DATA_BINDING = MarshallingInfo.builder(MarshallingType.JSON_VALUE).marshallLocation(MarshallLocation.PAYLOAD)
    ...
    public void marshall(Event event, ProtocolMarshaller protocolMarshaller) {
        ...
        try {
            protocolMarshaller.marshall(event.getData(), DATA_BINDING);
            protocolMarshaller.marshall(event.getTimestamp(), TIMESTAMP_BINDING);
            protocolMarshaller.marshall(event.getType(), TYPE_BINDING);
        } catch (Exception e) {
