...
public class CopySnapshotRequest extends AmazonWebServiceRequest implements
     ...
     * <note>
     * <p>
     * <code>CopySnapshot</code> sends the snapshot copy to the regional
     * endpoint that you send the HTTP request to, such as
     * <code>ec2.us-east-1.amazonaws.com</code> (in the AWS CLI, this is
     * specified with the <code>--region</code> parameter or the default region
     ...
     * other cases. The <code>PresignedUrl</code> should use the snapshot source
     ...
     * a123-456a-a12b-a123b4cd56ef</i>. The specified CMK must exist in the
     * region that the snapshot is being copied to. If a <code>KmsKeyId</code>
     ...
     *        endpoint that you send the HTTP request to, such as
     *        <code>ec2.us-east-1.amazonaws.com</code> (in the AWS CLI, this is
     *        specified with the <code>--region</code> parameter or the default
     ...
     *        parameter in all other cases. The <code>PresignedUrl</code> should
     *        use the snapshot source endpoint, the <code>CopySnapshot</code>
     *        action, and include the <code>SourceRegion</code>,
     ...
     *         <code>PresignedUrl</code> should use the snapshot source
     *         endpoint, the <code>CopySnapshot</code> action, and include the
     *         <code>SourceRegion</code>, <code>SourceSnapshotId</code>, and
     ...
     *        contains the <code>arn:aws:kms</code> namespace, followed by the
     *        region of the CMK, the AWS account ID of the CMK owner, the
     *        <code>key</code> namespace, and then the CMK ID. For example,
     *        arn:aws
     *        :kms:<i>us-east-1</i>:<i>012345678910</i>:key/<i>abcd1234-a123
     ...
     *         <code>key</code> namespace, and then the CMK ID. For example,
     *         arn:aws:kms:<i>us-east-1</i>:<i>012345678910</i>:key/<i>abcd1234-
     *         a123-456a-a12b-a123b4cd56ef</i>. The specified CMK must exist in
    ...
    public Request<CopySnapshotRequest> getDryRunRequest() {
        Request<CopySnapshotRequest> request = new CopySnapshotRequestMarshaller()
                .marshall(this);
