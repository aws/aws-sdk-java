...
public class SearchStatusJsonUnmarshaller implements Unmarshaller<SearchStatus, JsonUnmarshallerContext> {
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("CompletionTimestamp", targetDepth)) {
                    context.nextToken();
                    searchStatus.setCompletionTimestamp(DateJsonUnmarshallerFactory.getInstance("unixTimestamp").unmarshall(context));
                }
                if (context.testExpression("ErrorMessage", targetDepth)) {
                    context.nextToken();
                    searchStatus.setErrorMessage(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("State", targetDepth)) {
                    context.nextToken();
                    searchStatus.setState(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("SubmissionTimestamp", targetDepth)) {
                    context.nextToken();
                    searchStatus.setSubmissionTimestamp(DateJsonUnmarshallerFactory.getInstance("unixTimestamp").unmarshall(context));
                }
    ...
    public static SearchStatusJsonUnmarshaller getInstance() {
        if (instance == null)
            instance = new SearchStatusJsonUnmarshaller();
        return instance;
    }
}
