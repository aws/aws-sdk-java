...
public class DeleteChannelRequestProtocolMarshaller implements Marshaller<Request<DeleteChannelRequest>, DeleteChannelRequest> {
    ...
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().protocol(Protocol.REST_JSON).requestUri("/channels/{id}")
            .httpMethodName(HttpMethodName.DELETE).hasExplicitPayloadMember(false).hasPayloadMembers(false).serviceName("AWSMediaPackage").build();
