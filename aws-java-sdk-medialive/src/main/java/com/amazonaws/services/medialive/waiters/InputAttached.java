...
class InputAttached {
    ...
    static class IsATTACHEDMatcher extends WaiterAcceptor<DescribeInputResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"ATTACHED\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }

        private static final JmesPathExpression ast = new JmesPathField("State");
         ...
         */
        @Override
        public boolean matches(DescribeInputResult result) {
    ...
    static class IsDETACHEDMatcher extends WaiterAcceptor<DescribeInputResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"DETACHED\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }
