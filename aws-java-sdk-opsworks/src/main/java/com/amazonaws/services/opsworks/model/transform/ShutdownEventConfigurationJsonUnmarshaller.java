...
/**
 * Shutdown Event Configuration JSON Unmarshaller
 */
public class ShutdownEventConfigurationJsonUnmarshaller implements Unmarshaller<ShutdownEventConfiguration, JsonUnmarshallerContext> {
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("ExecutionTimeout", targetDepth)) {
                    context.nextToken();
                    shutdownEventConfiguration.setExecutionTimeout(IntegerJsonUnmarshaller.getInstance().unmarshall(context));
                }
                if (context.testExpression("DelayUntilElbConnectionsDrained", targetDepth)) {
                    context.nextToken();
                    shutdownEventConfiguration.setDelayUntilElbConnectionsDrained(BooleanJsonUnmarshaller.getInstance().unmarshall(context));
                }
    ...
    public static ShutdownEventConfigurationJsonUnmarshaller getInstance() {
        if (instance == null) instance = new ShutdownEventConfigurationJsonUnmarshaller();
        return instance;
    }
}
