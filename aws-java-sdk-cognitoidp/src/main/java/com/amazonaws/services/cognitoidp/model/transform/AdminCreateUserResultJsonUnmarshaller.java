...
/**
 * AdminCreateUserResult JSON Unmarshaller
 */
...
public class AdminCreateUserResultJsonUnmarshaller implements Unmarshaller<AdminCreateUserResult, JsonUnmarshallerContext> {
    ...
    public AdminCreateUserResult unmarshall(JsonUnmarshallerContext context) throws Exception {
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("User", targetDepth)) {
                    context.nextToken();
                    adminCreateUserResult.setUser(UserTypeJsonUnmarshaller.getInstance().unmarshall(context));
                }
