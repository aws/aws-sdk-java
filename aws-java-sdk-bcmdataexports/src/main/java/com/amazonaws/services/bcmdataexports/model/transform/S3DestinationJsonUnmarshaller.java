        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("S3Bucket", targetDepth)) {
                    context.nextToken();
                    s3Destination.setS3Bucket(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("S3OutputConfigurations", targetDepth)) {
                    context.nextToken();
                    s3Destination.setS3OutputConfigurations(S3OutputConfigurationsJsonUnmarshaller.getInstance().unmarshall(context));
                }
                if (context.testExpression("S3Prefix", targetDepth)) {
                    context.nextToken();
                    s3Destination.setS3Prefix(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("S3Region", targetDepth)) {
                    context.nextToken();
                    s3Destination.setS3Region(context.getUnmarshaller(String.class).unmarshall(context));
                }
