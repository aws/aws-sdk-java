        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("Ipv6Support", targetDepth)) {
                    context.nextToken();
                    vpcOptions.setIpv6Support(context.getUnmarshaller(Boolean.class).unmarshall(context));
                }
                if (context.testExpression("ApplianceModeSupport", targetDepth)) {
                    context.nextToken();
                    vpcOptions.setApplianceModeSupport(context.getUnmarshaller(Boolean.class).unmarshall(context));
                }
