        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("type", targetDepth)) {
                    context.nextToken();
                    destinationConfiguration.setType(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("s3Configuration", targetDepth)) {
                    context.nextToken();
                    destinationConfiguration.setS3Configuration(S3DestinationConfigurationJsonUnmarshaller.getInstance().unmarshall(context));
                }
                if (context.testExpression("iotTwinMakerConfiguration", targetDepth)) {
                    context.nextToken();
                    destinationConfiguration.setIotTwinMakerConfiguration(IotTwinMakerDestinationConfigurationJsonUnmarshaller.getInstance()
                            .unmarshall(context));
                }
