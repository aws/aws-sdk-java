        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("Value", targetDepth)) {
                    context.nextToken();
                    tableCell.setValue(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("TopAnswer", targetDepth)) {
                    context.nextToken();
                    tableCell.setTopAnswer(context.getUnmarshaller(Boolean.class).unmarshall(context));
                }
                if (context.testExpression("Highlighted", targetDepth)) {
                    context.nextToken();
                    tableCell.setHighlighted(context.getUnmarshaller(Boolean.class).unmarshall(context));
                }
                if (context.testExpression("Header", targetDepth)) {
                    context.nextToken();
                    tableCell.setHeader(context.getUnmarshaller(Boolean.class).unmarshall(context));
                }
