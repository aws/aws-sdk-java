...
/**
 * Set Queue Attributes Request StAX Unmarshaller
 */
public class SetQueueAttributesRequestStaxUnmarshaller implements Unmarshaller<SetQueueAttributesRequest, StaxUnmarshallerContext> {
    ...
    public SetQueueAttributesRequest unmarshall(StaxUnmarshallerContext context) throws Exception {
        ...
        while (true) {
            XMLEvent event = context.nextEvent();
            if (event.isEndDocument()) return setQueueAttributesRequest;

            if (event.isAttribute() || event.isStartElement()) {
                if (context.testExpression("QueueUrl", targetDepth)) {
                    setQueueAttributesRequest.setQueueUrl(StringStaxUnmarshaller.getInstance().unmarshall(context));
                    continue;
                }
                if (context.testExpression("Attribute", targetDepth)) {
                    Entry<String, String> entry = AttributesMapEntryUnmarshaller.getInstance().unmarshall(context);
                    setQueueAttributesRequest.getAttributes().put(entry.getKey(), entry.getValue());
                    continue;
                }
            } else if (event.isEndElement()) {
    ...
    private static SetQueueAttributesRequestStaxUnmarshaller instance;
    public static SetQueueAttributesRequestStaxUnmarshaller getInstance() {
        if (instance == null) instance = new SetQueueAttributesRequestStaxUnmarshaller();
        return instance;
    }
}
