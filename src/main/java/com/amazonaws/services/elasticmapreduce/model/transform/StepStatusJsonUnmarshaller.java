 ...
 */
public class StepStatusJsonUnmarshaller implements Unmarshaller<StepStatus, JsonUnmarshallerContext> {
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("State", targetDepth)) {
                    context.nextToken();
                    stepStatus.setState(StringJsonUnmarshaller.getInstance().unmarshall(context));
                }
    ...
    public static StepStatusJsonUnmarshaller getInstance() {
        if (instance == null) instance = new StepStatusJsonUnmarshaller();
        return instance;
    }
}
