...
public class BucketJsonUnmarshaller implements Unmarshaller<Bucket, JsonUnmarshallerContext> {
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("value", targetDepth)) {
                    context.nextToken();
                    bucket.setValue(StringJsonUnmarshaller.getInstance().unmarshall(context));
                }
                if (context.testExpression("count", targetDepth)) {
                    context.nextToken();
                    bucket.setCount(LongJsonUnmarshaller.getInstance().unmarshall(context));
                }
    ...
    public static BucketJsonUnmarshaller getInstance() {
        if (instance == null) instance = new BucketJsonUnmarshaller();
        return instance;
    }
}
