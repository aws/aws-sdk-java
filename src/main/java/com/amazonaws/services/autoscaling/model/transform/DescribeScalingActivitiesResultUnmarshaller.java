...
public class DescribeScalingActivitiesResultUnmarshaller implements Unmarshaller<DescribeScalingActivitiesResult, Node> {
    ...
    public DescribeScalingActivitiesResult unmarshall(Node node) throws Exception {
        ...
        for (int activitiesIndex = 0; activitiesIndex < XpathUtils.nodeLength(activitiesNodes); ++activitiesIndex) {
            Node activitiesNode = activitiesNodes.item(activitiesIndex);
            describeScalingActivitiesResult.getActivities().add(new ActivityUnmarshaller().unmarshall(activitiesNode));
            activitiesNode.getParentNode().removeChild(activitiesNode);
        }
    
        Node nextTokenNode = XpathUtils.asNode("NextToken", node);
        describeScalingActivitiesResult.setNextToken(new StringUnmarshaller().unmarshall(nextTokenNode));
