...
public class ListDomainsResultUnmarshaller implements Unmarshaller<ListDomainsResult, Node> {
    ...
    public ListDomainsResult unmarshall(Node node) throws Exception {
        ...
        for (int domainNamesIndex = 0; domainNamesIndex < XpathUtils.nodeLength(domainNamesNodes); ++domainNamesIndex) {
            Node domainNamesNode = domainNamesNodes.item(domainNamesIndex);
            listDomainsResult.getDomainNames().add(new StringUnmarshaller().unmarshall(domainNamesNode));
            domainNamesNode.getParentNode().removeChild(domainNamesNode);
        }
    
        Node nextTokenNode = XpathUtils.asNode("NextToken", node);
        listDomainsResult.setNextToken(new StringUnmarshaller().unmarshall(nextTokenNode));
