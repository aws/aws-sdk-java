...
public class DynamoDBv2ActionJsonUnmarshaller implements Unmarshaller<DynamoDBv2Action, JsonUnmarshallerContext> {
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("roleArn", targetDepth)) {
                    context.nextToken();
                    dynamoDBv2Action.setRoleArn(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("putItem", targetDepth)) {
                    context.nextToken();
                    dynamoDBv2Action.setPutItem(PutItemInputJsonUnmarshaller.getInstance().unmarshall(context));
                }
