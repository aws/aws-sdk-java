...
public class UpdateServiceRequestProtocolMarshaller implements Marshaller<Request<UpdateServiceRequest>, UpdateServiceRequest> {
    ...
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().protocol(Protocol.REST_JSON).requestUri("/services/{serviceIdentifier}")
            .httpMethodName(HttpMethodName.PATCH).hasExplicitPayloadMember(false).hasPayloadMembers(true).serviceName("AmazonVPCLattice").build();
