...
class GroupNotExists {
    ...
    static class IsFalseMatcher extends WaiterAcceptor<DescribeAutoScalingGroupsResult> {
        ...
        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("false");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }

        private static final JmesPathExpression ast = new OpGreaterThan(new JmesPathLengthFunction(new JmesPathField("AutoScalingGroups")),
