        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("DataSourceS3Configuration", targetDepth)) {
                    context.nextToken();
                    dataSourceConfiguration.setDataSourceS3Configuration(DataSourceS3ConfigurationJsonUnmarshaller.getInstance().unmarshall(context));
                }
                if (context.testExpression("CsvConfiguration", targetDepth)) {
                    context.nextToken();
                    dataSourceConfiguration.setCsvConfiguration(CsvConfigurationJsonUnmarshaller.getInstance().unmarshall(context));
                }
                if (context.testExpression("DataFormat", targetDepth)) {
                    context.nextToken();
                    dataSourceConfiguration.setDataFormat(context.getUnmarshaller(String.class).unmarshall(context));
                }
