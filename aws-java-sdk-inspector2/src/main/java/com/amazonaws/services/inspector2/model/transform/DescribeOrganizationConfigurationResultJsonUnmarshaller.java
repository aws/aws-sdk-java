        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("autoEnable", targetDepth)) {
                    context.nextToken();
                    describeOrganizationConfigurationResult.setAutoEnable(AutoEnableJsonUnmarshaller.getInstance().unmarshall(context));
                }
                if (context.testExpression("maxAccountLimitReached", targetDepth)) {
                    context.nextToken();
                    describeOrganizationConfigurationResult.setMaxAccountLimitReached(context.getUnmarshaller(Boolean.class).unmarshall(context));
                }
