        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("failed", targetDepth)) {
                    context.nextToken();
                    instanceProperty.setFailed(context.getUnmarshaller(Boolean.class).unmarshall(context));
                }
                if (context.testExpression("properties", targetDepth)) {
                    context.nextToken();
                    instanceProperty.setProperties(new MapUnmarshaller<String, String>(context.getUnmarshaller(String.class), context
                            .getUnmarshaller(String.class)).unmarshall(context));
                }
                if (context.testExpression("seenAt", targetDepth)) {
                    context.nextToken();
                    instanceProperty.setSeenAt(DateJsonUnmarshallerFactory.getInstance("iso8601").unmarshall(context));
                }
