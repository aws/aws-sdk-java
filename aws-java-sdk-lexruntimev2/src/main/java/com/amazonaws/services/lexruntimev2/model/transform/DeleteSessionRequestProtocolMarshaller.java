...
public class DeleteSessionRequestProtocolMarshaller implements Marshaller<Request<DeleteSessionRequest>, DeleteSessionRequest> {
    ...
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().protocol(Protocol.REST_JSON)
            .requestUri("/bots/{botId}/botAliases/{botAliasId}/botLocales/{localeId}/sessions/{sessionId}").httpMethodName(HttpMethodName.DELETE)
            .hasExplicitPayloadMember(false).hasPayloadMembers(false).serviceName("AmazonLexRuntimeV2").build();
