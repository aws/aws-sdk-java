...
public class ListDevicesResultJsonUnmarshaller implements Unmarshaller<ListDevicesResult, JsonUnmarshallerContext> {
    ...
    public ListDevicesResult unmarshall(JsonUnmarshallerContext context) throws Exception {
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("devices", targetDepth)) {
                    context.nextToken();
                    listDevicesResult.setDevices(new ListUnmarshaller<DeviceSummary>(DeviceSummaryJsonUnmarshaller.getInstance())

                    .unmarshall(context));
                }
                if (context.testExpression("nextToken", targetDepth)) {
                    context.nextToken();
                    listDevicesResult.setNextToken(context.getUnmarshaller(String.class).unmarshall(context));
                }
