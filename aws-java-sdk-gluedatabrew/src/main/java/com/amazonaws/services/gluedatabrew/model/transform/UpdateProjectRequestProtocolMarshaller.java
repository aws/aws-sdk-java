...
public class UpdateProjectRequestProtocolMarshaller implements Marshaller<Request<UpdateProjectRequest>, UpdateProjectRequest> {
    ...
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().protocol(Protocol.REST_JSON).requestUri("/projects/{name}")
            .httpMethodName(HttpMethodName.PUT).hasExplicitPayloadMember(false).hasPayloadMembers(true).serviceName("AWSGlueDataBrew").build();
