...
public class StartJobRunRequestProtocolMarshaller implements Marshaller<Request<StartJobRunRequest>, StartJobRunRequest> {
    ...
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().protocol(Protocol.REST_JSON).requestUri("/jobs/{name}/startJobRun")
            .httpMethodName(HttpMethodName.POST).hasExplicitPayloadMember(false).hasPayloadMembers(false).serviceName("AWSGlueDataBrew").build();
