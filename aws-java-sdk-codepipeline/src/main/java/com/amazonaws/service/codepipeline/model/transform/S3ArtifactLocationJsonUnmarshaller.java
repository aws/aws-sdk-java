 ...
 */
public class S3ArtifactLocationJsonUnmarshaller implements
        Unmarshaller<S3ArtifactLocation, JsonUnmarshallerContext> {

    public S3ArtifactLocation unmarshall(JsonUnmarshallerContext context)
            throws Exception {
        S3ArtifactLocation s3ArtifactLocation = new S3ArtifactLocation();
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("bucketName", targetDepth)) {
                    context.nextToken();
                    s3ArtifactLocation.setBucketName(StringJsonUnmarshaller
                            .getInstance().unmarshall(context));
                }
                if (context.testExpression("objectKey", targetDepth)) {
                    context.nextToken();
                    s3ArtifactLocation.setObjectKey(StringJsonUnmarshaller
                            .getInstance().unmarshall(context));
                }
    ...
    public static S3ArtifactLocationJsonUnmarshaller getInstance() {
        if (instance == null)
            instance = new S3ArtifactLocationJsonUnmarshaller();
        return instance;
    }
}
