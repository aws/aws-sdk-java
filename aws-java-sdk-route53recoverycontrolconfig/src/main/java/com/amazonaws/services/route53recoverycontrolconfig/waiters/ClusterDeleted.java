...
class ClusterDeleted {
    ...
    static class IsPENDING_DELETIONMatcher extends WaiterAcceptor<DescribeClusterResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"PENDING_DELETION\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }

        private static final JmesPathExpression ast = new JmesPathSubExpression(new JmesPathField("Cluster"), new JmesPathField("Status"));
