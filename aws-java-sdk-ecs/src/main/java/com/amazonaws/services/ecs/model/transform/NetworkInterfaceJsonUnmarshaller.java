        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("attachmentId", targetDepth)) {
                    context.nextToken();
                    networkInterface.setAttachmentId(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("privateIpv4Address", targetDepth)) {
                    context.nextToken();
                    networkInterface.setPrivateIpv4Address(context.getUnmarshaller(String.class).unmarshall(context));
                }
