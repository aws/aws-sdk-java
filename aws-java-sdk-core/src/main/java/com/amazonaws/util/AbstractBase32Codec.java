...
abstract class AbstractBase32Codec implements Codec {
    ...
    private final void encode5bytes(byte[] src, int s, byte[] dest, int d) {
        ...
        byte p;
        dest[d++] = (byte)ALPAHBETS[(p=src[s++]) >>> 3 & MASK_5BITS];                         // 5 
        dest[d++] = (byte)ALPAHBETS[(p & MASK_3BITS) << 2 | (p=src[s++]) >>> 6 & MASK_2BITS]; // 3 2
        dest[d++] = (byte)ALPAHBETS[p >>> 1 & MASK_5BITS];                                    //   5
        dest[d++] = (byte)ALPAHBETS[(p & 1) << 4 | (p=src[s++]) >>> 4 & MASK_4BITS];          //   1 4
        dest[d++] = (byte)ALPAHBETS[(p & MASK_4BITS) << 1 | (p=src[s++]) >>> 7 & 1];          //     4 1
        dest[d++] = (byte)ALPAHBETS[p >>> 2 & MASK_5BITS];                                    //       5
        dest[d++] = (byte)ALPAHBETS[(p & MASK_2BITS) << 3 | (p=src[s]) >>> 5 & MASK_3BITS];   //       2 3
        dest[d] = (byte)ALPAHBETS[p & MASK_5BITS];                                            //         5
        return;
    }
    ...
    private final void encode4bytes(byte[] src, int s, byte[] dest, int d) {
        ...
        dest[d++] = (byte)ALPAHBETS[(p & MASK_4BITS) << 1 | (p=src[s]) >>> 7 & 1];            //     4 1
        dest[d++] = (byte)ALPAHBETS[p >>> 2 & MASK_5BITS];                                    //       5
        dest[d++] = (byte)ALPAHBETS[(p & MASK_2BITS) << 3];                                   //       2
        dest[d] = PAD;
        return;
    }
    ...
    private final void encode3bytes(byte[] src, int s, byte[] dest, int d) {
        ...
        dest[d++] = (byte)ALPAHBETS[p >>> 1 & MASK_5BITS];                                    //   5
        dest[d++] = (byte)ALPAHBETS[(p & 1) << 4 | (p=src[s]) >>> 4 & MASK_4BITS];            //   1 4
        dest[d++] = (byte)ALPAHBETS[(p & MASK_4BITS) << 1];                                   //     4
    ...
    private final void encode1byte(byte[] src, int s, byte[] dest, int d) {
        ...
        byte p;
        dest[d++] = (byte)ALPAHBETS[(p=src[s]) >>> 3 & MASK_5BITS];                            // 5 
        dest[d++] = (byte)ALPAHBETS[(p & MASK_3BITS) << 2];                                    // 3
