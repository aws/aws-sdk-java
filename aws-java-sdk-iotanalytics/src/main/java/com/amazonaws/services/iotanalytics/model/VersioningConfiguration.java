/*
 * Copyright 2014-2019 Amazon.com, Inc. or its affiliates. All Rights Reserved.
 * 
 * Licensed under the Apache License, Version 2.0 (the "License"). You may not use this file except in compliance with
 * the License. A copy of the License is located at
 * 
 * http://aws.amazon.com/apache2.0
 * 
 * or in the "license" file accompanying this file. This file is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR
 * CONDITIONS OF ANY KIND, either express or implied. See the License for the specific language governing permissions
 * and limitations under the License.
 */
package com.amazonaws.services.iotanalytics.model;

import java.io.Serializable;
import javax.annotation.Generated;
import com.amazonaws.protocol.StructuredPojo;
import com.amazonaws.protocol.ProtocolMarshaller;

/**
 * 
 * @see <a href="http://docs.aws.amazon.com/goto/WebAPI/iotanalytics-2017-11-27/VersioningConfiguration"
 *      target="_top">AWS API Documentation</a>
 */
@Generated("com.amazonaws:aws-java-sdk-code-generator")
public class VersioningConfiguration implements Serializable, Cloneable, StructuredPojo {

    /**
     * <p>
     * If true, unlimited versions of data set contents will be kept.
     * </p>
     */
    private Boolean unlimited;
    /**
     * <p>
     * How many versions of data set contents will be kept. The "unlimited" parameter must be false.
     * </p>
     */
    private Integer maxVersions;

    /**
     * <p>
     * If true, unlimited versions of data set contents will be kept.
     * </p>
     * 
     * @param unlimited
     *        If true, unlimited versions of data set contents will be kept.
     */

    public void setUnlimited(Boolean unlimited) {
        this.unlimited = unlimited;
    }

    /**
     * <p>
     * If true, unlimited versions of data set contents will be kept.
     * </p>
     * 
     * @return If true, unlimited versions of data set contents will be kept.
     */

    public Boolean getUnlimited() {
        return this.unlimited;
    }

    /**
     * <p>
     * If true, unlimited versions of data set contents will be kept.
     * </p>
     * 
     * @param unlimited
     *        If true, unlimited versions of data set contents will be kept.
     * @return Returns a reference to this object so that method calls can be chained together.
     */

    public VersioningConfiguration withUnlimited(Boolean unlimited) {
        setUnlimited(unlimited);
        return this;
    }

    /**
     * <p>
     * If true, unlimited versions of data set contents will be kept.
     * </p>
     * 
     * @return If true, unlimited versions of data set contents will be kept.
     */

    public Boolean isUnlimited() {
        return this.unlimited;
    }

    /**
     * <p>
     * How many versions of data set contents will be kept. The "unlimited" parameter must be false.
     * </p>
     * 
     * @param maxVersions
     *        How many versions of data set contents will be kept. The "unlimited" parameter must be false.
     */

    public void setMaxVersions(Integer maxVersions) {
        this.maxVersions = maxVersions;
    }

    /**
     * <p>
     * How many versions of data set contents will be kept. The "unlimited" parameter must be false.
     * </p>
     * 
     * @return How many versions of data set contents will be kept. The "unlimited" parameter must be false.
     */

    public Integer getMaxVersions() {
        return this.maxVersions;
    }

    /**
     * <p>
     * How many versions of data set contents will be kept. The "unlimited" parameter must be false.
     * </p>
     * 
     * @param maxVersions
     *        How many versions of data set contents will be kept. The "unlimited" parameter must be false.
     * @return Returns a reference to this object so that method calls can be chained together.
     */

    public VersioningConfiguration withMaxVersions(Integer maxVersions) {
        setMaxVersions(maxVersions);
        return this;
    }

    /**
     * Returns a string representation of this object. This is useful for testing and debugging. Sensitive data will be
     * redacted from this string using a placeholder value.
     *
     * @return A string representation of this object.
     *
     * @see java.lang.Object#toString()
     */
    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder();
        sb.append("{");
        if (getUnlimited() != null)
            sb.append("Unlimited: ").append(getUnlimited()).append(",");
        if (getMaxVersions() != null)
            sb.append("MaxVersions: ").append(getMaxVersions());
        sb.append("}");
        return sb.toString();
    }

    @Override
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (obj == null)
            return false;

        if (obj instanceof VersioningConfiguration == false)
            return false;
        VersioningConfiguration other = (VersioningConfiguration) obj;
        if (other.getUnlimited() == null ^ this.getUnlimited() == null)
            return false;
        if (other.getUnlimited() != null && other.getUnlimited().equals(this.getUnlimited()) == false)
            return false;
        if (other.getMaxVersions() == null ^ this.getMaxVersions() == null)
            return false;
        if (other.getMaxVersions() != null && other.getMaxVersions().equals(this.getMaxVersions()) == false)
            return false;
        return true;
    }

    @Override
    public int hashCode() {
        final int prime = 31;
        int hashCode = 1;

        hashCode = prime * hashCode + ((getUnlimited() == null) ? 0 : getUnlimited().hashCode());
        hashCode = prime * hashCode + ((getMaxVersions() == null) ? 0 : getMaxVersions().hashCode());
        return hashCode;
    }

    @Override
    public VersioningConfiguration clone() {
        try {
            return (VersioningConfiguration) super.clone();
        } catch (CloneNotSupportedException e) {
            throw new IllegalStateException("Got a CloneNotSupportedException from Object.clone() " + "even though we're Cloneable!", e);
        }
    }

    @com.amazonaws.annotation.SdkInternalApi
    @Override
    public void marshall(ProtocolMarshaller protocolMarshaller) {
        com.amazonaws.services.iotanalytics.model.transform.VersioningConfigurationMarshaller.getInstance().marshall(this, protocolMarshaller);
    }
}
