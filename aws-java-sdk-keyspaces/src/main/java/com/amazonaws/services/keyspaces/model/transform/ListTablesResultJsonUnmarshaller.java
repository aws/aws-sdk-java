        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("nextToken", targetDepth)) {
                    context.nextToken();
                    listTablesResult.setNextToken(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("tables", targetDepth)) {
                    context.nextToken();
                    listTablesResult.setTables(new ListUnmarshaller<TableSummary>(TableSummaryJsonUnmarshaller.getInstance())
