...
class DynamoDBReflector {
    ...
    private final Map<Method, AttributeAction> attributeSaveActionCache = new HashMap<Method, AttributeAction>();
    ...
    /**
     * Returns the attribute's saveAction corresponding to the given getter method.
     */
    AttributeAction getAttributeSaveAction(Method getter) {
        synchronized (attributeSaveActionCache) {
            if ( !attributeSaveActionCache.containsKey(getter) ) {
                DynamoDBAttribute attribute = ReflectionUtils.getAnnotationFromGetterOrField(getter, DynamoDBAttribute.class);
                attributeSaveActionCache.put(getter,attribute.saveAction());
            }
            return attributeSaveActionCache.get(getter);
        }
    }
