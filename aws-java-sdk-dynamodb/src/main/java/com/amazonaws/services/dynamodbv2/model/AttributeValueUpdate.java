 ...
 * <note>
 * <p>
 * You cannot use <i>UpdateItem</i> to update any primary key attributes.
 * Instead, you will need to delete the item, and then use <i>PutItem</i> to
...
public class AttributeValueUpdate implements Serializable, Cloneable {
     ...
     * <li>
     * <p>
     * <code>DELETE</code> - If no value is specified, the attribute and its
     ...
     * not exist. DynamoDB will create the <i>itemcount</i> attribute, set its
     * initial value to <code>0</code>, and finally add <code>3</code> to it.
     * The result will be a new <i>itemcount</i> attribute in the item, with a
     ...
     * action specified <code>[3]</code>, then the final attribute value would
     * be <code>[1,2,3]</code>. An error occurs if an Add action is specified
     ...
     *        <code>ADD</code>. The behavior depends on whether the specified
     *        primary key already exists in the table.</p>
     *        <p>
     *        <b>If an item with the specified <i>Key</i> is found in the
     *        table:</b>
     *        </p>
     *        <ul>
     *        <li>
     ...
     *        <li>
     *        <p>
     *        <code>DELETE</code> - If no value is specified, the attribute and
     ...
     *        was the set <code>[a,b,c]</code> and the <i>DELETE</i> action
     *        specified <code>[a,c]</code>, then the final attribute value would
     *        be <code>[b]</code>. Specifying an empty set is an error.
     *        </p>
     *        </li>
     *        <li>
     *        <p>
     *        <code>ADD</code> - If the attribute does not already exist, then
     *        the attribute and its values are added to the item. If the
     *        attribute does exist, then the behavior of <code>ADD</code>
     ...
     *        a number, then the <i>Value</i> is mathematically added to the
     ...
     *        </p>
     *        <p>
     *        In addition, if you use <code>ADD</code> to update an existing
     *        item, and intend to increment or decrement an attribute value
     *        which does not yet exist, DynamoDB uses <code>0</code> as the
     *        initial value. For example, suppose that the item you want to
     *        update does not yet have an attribute named <i>itemcount</i>, but
     *        you decide to <code>ADD</code> the number <code>3</code> to this
     *        attribute anyway, even though it currently does not exist.
     *        DynamoDB will create the <i>itemcount</i> attribute, set its
     *        initial value to <code>0</code>, and finally add <code>3</code> to
     *        it. The result will be a new <i>itemcount</i> attribute in the
     *        item, with a value of <code>3</code>.
     *        </p>
     *        </note></li>
     *        <li>
     *        <p>
     *        If the existing data type is a set, and if the <i>Value</i> is
     *        also a set, then the <i>Value</i> is added to the existing set.
     *        (This is a <i>set</i> operation, not mathematical addition.) For
     *        example, if the attribute value was the set <code>[1,2]</code>,
     *        and the <code>ADD</code> action specified <code>[3]</code>, then
     *        the final attribute value would be <code>[1,2,3]</code>. An error
     ...
     *        type is number or is a set. Do not use <code>ADD</code> for any
     *        other data types.
     *        </p>
     *        </li>
     *        </ul>
     ...
     *        <li>
     *        <p>
     *        <code>PUT</code> - DynamoDB creates a new item with the specified
     *        primary key, and then adds the attribute.
     *        </p>
     *        </li>
     *        <li>
     *        <p>
     *        <code>DELETE</code> - Nothing happens; there is no attribute to
     *        delete.
     *        </p>
     *        </li>
     *        <li>
     *        <p>
     *        <code>ADD</code> - DynamoDB creates an item with the supplied
     ...
     *         subtracted from the old set. For example, if the attribute value
     *         was the set <code>[a,b,c]</code> and the <i>DELETE</i> action
     *         specified <code>[a,c]</code>, then the final attribute value
     ...
     *         also a number, then the <i>Value</i> is mathematically added to
     ...
     *         attribute anyway, even though it currently does not exist.
     *         DynamoDB will create the <i>itemcount</i> attribute, set its
     *         initial value to <code>0</code>, and finally add <code>3</code>
     *         to it. The result will be a new <i>itemcount</i> attribute in the
     *         item, with a value of <code>3</code>.
     *         </p>
     *         </note></li>
     *         <li>
