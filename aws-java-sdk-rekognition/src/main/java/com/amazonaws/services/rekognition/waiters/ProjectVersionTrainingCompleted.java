...
class ProjectVersionTrainingCompleted {
    ...
    static class IsTRAINING_COMPLETEDMatcher extends WaiterAcceptor<DescribeProjectVersionsResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"TRAINING_COMPLETED\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }

        private static final JmesPathExpression ast = new JmesPathProjection(new JmesPathFlatten(new JmesPathField("ProjectVersionDescriptions")),
         ...
         */
        @Override
        public boolean matches(DescribeProjectVersionsResult result) {
    ...
    static class IsTRAINING_FAILEDMatcher extends WaiterAcceptor<DescribeProjectVersionsResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"TRAINING_FAILED\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }
