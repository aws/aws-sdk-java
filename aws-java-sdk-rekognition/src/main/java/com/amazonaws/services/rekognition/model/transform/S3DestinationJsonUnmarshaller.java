...
public class S3DestinationJsonUnmarshaller implements Unmarshaller<S3Destination, JsonUnmarshallerContext> {
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("Bucket", targetDepth)) {
                    context.nextToken();
                    s3Destination.setBucket(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("KeyPrefix", targetDepth)) {
                    context.nextToken();
                    s3Destination.setKeyPrefix(context.getUnmarshaller(String.class).unmarshall(context));
                }
