        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("GroupLifecycleEventsDesiredStatus", targetDepth)) {
                    context.nextToken();
                    accountSettings.setGroupLifecycleEventsDesiredStatus(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("GroupLifecycleEventsStatus", targetDepth)) {
                    context.nextToken();
                    accountSettings.setGroupLifecycleEventsStatus(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("GroupLifecycleEventsStatusMessage", targetDepth)) {
                    context.nextToken();
                    accountSettings.setGroupLifecycleEventsStatusMessage(context.getUnmarshaller(String.class).unmarshall(context));
                }
