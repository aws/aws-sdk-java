...
public class GetGroupRequestProtocolMarshaller implements Marshaller<Request<GetGroupRequest>, GetGroupRequest> {
    ...
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().protocol(Protocol.REST_JSON).requestUri("/group/{groupIdentifier}")
            .httpMethodName(HttpMethodName.GET).hasExplicitPayloadMember(false).hasPayloadMembers(false).serviceName("AWSSynthetics").build();
