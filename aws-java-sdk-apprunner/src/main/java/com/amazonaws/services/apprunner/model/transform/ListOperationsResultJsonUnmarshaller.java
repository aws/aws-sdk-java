        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("OperationSummaryList", targetDepth)) {
                    context.nextToken();
                    listOperationsResult.setOperationSummaryList(new ListUnmarshaller<OperationSummary>(OperationSummaryJsonUnmarshaller.getInstance())

                    .unmarshall(context));
                }
                if (context.testExpression("NextToken", targetDepth)) {
                    context.nextToken();
                    listOperationsResult.setNextToken(context.getUnmarshaller(String.class).unmarshall(context));
                }
