/*
 * Copyright 2019-2024 Amazon.com, Inc. or its affiliates. All Rights Reserved.
 * 
 * Licensed under the Apache License, Version 2.0 (the "License"). You may not use this file except in compliance with
 * the License. A copy of the License is located at
 * 
 * http://aws.amazon.com/apache2.0
 * 
 * or in the "license" file accompanying this file. This file is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR
 * CONDITIONS OF ANY KIND, either express or implied. See the License for the specific language governing permissions
 * and limitations under the License.
 */
package com.amazonaws.services.eventbridge.model;

import java.io.Serializable;
import javax.annotation.Generated;
import com.amazonaws.protocol.StructuredPojo;
import com.amazonaws.protocol.ProtocolMarshaller;

/**
 * <p>
 * Contains the GraphQL operation to be parsed and executed, if the event target is an AppSync API.
 * </p>
 * 
 * @see <a href="http://docs.aws.amazon.com/goto/WebAPI/eventbridge-2015-10-07/AppSyncParameters" target="_top">AWS API
 *      Documentation</a>
 */
@Generated("com.amazonaws:aws-java-sdk-code-generator")
public class AppSyncParameters implements Serializable, Cloneable, StructuredPojo {

    /**
     * <p>
     * The GraphQL operation; that is, the query, mutation, or subscription to be parsed and executed by the GraphQL
     * service.
     * </p>
     * <p>
     * For more information, see <a
     * href="https://docs.aws.amazon.com/appsync/latest/devguide/graphql-architecture.html#graphql-operations"
     * >Operations</a> in the <i>AppSync User Guide</i>.
     * </p>
     */
    private String graphQLOperation;

    /**
     * <p>
     * The GraphQL operation; that is, the query, mutation, or subscription to be parsed and executed by the GraphQL
     * service.
     * </p>
     * <p>
     * For more information, see <a
     * href="https://docs.aws.amazon.com/appsync/latest/devguide/graphql-architecture.html#graphql-operations"
     * >Operations</a> in the <i>AppSync User Guide</i>.
     * </p>
     * 
     * @param graphQLOperation
     *        The GraphQL operation; that is, the query, mutation, or subscription to be parsed and executed by the
     *        GraphQL service.</p>
     *        <p>
     *        For more information, see <a
     *        href="https://docs.aws.amazon.com/appsync/latest/devguide/graphql-architecture.html#graphql-operations"
     *        >Operations</a> in the <i>AppSync User Guide</i>.
     */

    public void setGraphQLOperation(String graphQLOperation) {
        this.graphQLOperation = graphQLOperation;
    }

    /**
     * <p>
     * The GraphQL operation; that is, the query, mutation, or subscription to be parsed and executed by the GraphQL
     * service.
     * </p>
     * <p>
     * For more information, see <a
     * href="https://docs.aws.amazon.com/appsync/latest/devguide/graphql-architecture.html#graphql-operations"
     * >Operations</a> in the <i>AppSync User Guide</i>.
     * </p>
     * 
     * @return The GraphQL operation; that is, the query, mutation, or subscription to be parsed and executed by the
     *         GraphQL service.</p>
     *         <p>
     *         For more information, see <a
     *         href="https://docs.aws.amazon.com/appsync/latest/devguide/graphql-architecture.html#graphql-operations"
     *         >Operations</a> in the <i>AppSync User Guide</i>.
     */

    public String getGraphQLOperation() {
        return this.graphQLOperation;
    }

    /**
     * <p>
     * The GraphQL operation; that is, the query, mutation, or subscription to be parsed and executed by the GraphQL
     * service.
     * </p>
     * <p>
     * For more information, see <a
     * href="https://docs.aws.amazon.com/appsync/latest/devguide/graphql-architecture.html#graphql-operations"
     * >Operations</a> in the <i>AppSync User Guide</i>.
     * </p>
     * 
     * @param graphQLOperation
     *        The GraphQL operation; that is, the query, mutation, or subscription to be parsed and executed by the
     *        GraphQL service.</p>
     *        <p>
     *        For more information, see <a
     *        href="https://docs.aws.amazon.com/appsync/latest/devguide/graphql-architecture.html#graphql-operations"
     *        >Operations</a> in the <i>AppSync User Guide</i>.
     * @return Returns a reference to this object so that method calls can be chained together.
     */

    public AppSyncParameters withGraphQLOperation(String graphQLOperation) {
        setGraphQLOperation(graphQLOperation);
        return this;
    }

    /**
     * Returns a string representation of this object. This is useful for testing and debugging. Sensitive data will be
     * redacted from this string using a placeholder value.
     *
     * @return A string representation of this object.
     *
     * @see java.lang.Object#toString()
     */
    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder();
        sb.append("{");
        if (getGraphQLOperation() != null)
            sb.append("GraphQLOperation: ").append("***Sensitive Data Redacted***");
        sb.append("}");
        return sb.toString();
    }

    @Override
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (obj == null)
            return false;

        if (obj instanceof AppSyncParameters == false)
            return false;
        AppSyncParameters other = (AppSyncParameters) obj;
        if (other.getGraphQLOperation() == null ^ this.getGraphQLOperation() == null)
            return false;
        if (other.getGraphQLOperation() != null && other.getGraphQLOperation().equals(this.getGraphQLOperation()) == false)
            return false;
        return true;
    }

    @Override
    public int hashCode() {
        final int prime = 31;
        int hashCode = 1;

        hashCode = prime * hashCode + ((getGraphQLOperation() == null) ? 0 : getGraphQLOperation().hashCode());
        return hashCode;
    }

    @Override
    public AppSyncParameters clone() {
        try {
            return (AppSyncParameters) super.clone();
        } catch (CloneNotSupportedException e) {
            throw new IllegalStateException("Got a CloneNotSupportedException from Object.clone() " + "even though we're Cloneable!", e);
        }
    }

    @com.amazonaws.annotation.SdkInternalApi
    @Override
    public void marshall(ProtocolMarshaller protocolMarshaller) {
        com.amazonaws.services.eventbridge.model.transform.AppSyncParametersMarshaller.getInstance().marshall(this, protocolMarshaller);
    }
}
