...
public class TagResourceRequestProtocolMarshaller implements Marshaller<Request<TagResourceRequest>, TagResourceRequest> {
    ...
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().protocol(Protocol.REST_JSON).requestUri("/v20190125/tag")
            .httpMethodName(HttpMethodName.PUT).hasExplicitPayloadMember(false).hasPayloadMembers(true).serviceName("AWSAppMesh").build();
