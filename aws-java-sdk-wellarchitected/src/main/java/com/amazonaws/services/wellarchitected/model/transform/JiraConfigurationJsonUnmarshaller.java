    ...
    public JiraConfiguration unmarshall(JsonUnmarshallerContext context) throws Exception {
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("JiraIssueUrl", targetDepth)) {
                    context.nextToken();
                    jiraConfiguration.setJiraIssueUrl(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("LastSyncedTime", targetDepth)) {
                    context.nextToken();
                    jiraConfiguration.setLastSyncedTime(DateJsonUnmarshallerFactory.getInstance("unixTimestamp").unmarshall(context));
                }
