...
public class DiagnosticsJsonUnmarshaller implements Unmarshaller<Diagnostics, JsonUnmarshallerContext> {
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("errorCode", targetDepth)) {
                    context.nextToken();
                    diagnostics.setErrorCode(StringJsonUnmarshaller.getInstance().unmarshall(context));
                }
                if (context.testExpression("scriptName", targetDepth)) {
                    context.nextToken();
                    diagnostics.setScriptName(StringJsonUnmarshaller.getInstance().unmarshall(context));
                }
                if (context.testExpression("message", targetDepth)) {
                    context.nextToken();
                    diagnostics.setMessage(StringJsonUnmarshaller.getInstance().unmarshall(context));
                }
                if (context.testExpression("logTail", targetDepth)) {
                    context.nextToken();
                    diagnostics.setLogTail(StringJsonUnmarshaller.getInstance().unmarshall(context));
                }
    ...
    public static DiagnosticsJsonUnmarshaller getInstance() {
        if (instance == null) instance = new DiagnosticsJsonUnmarshaller();
        return instance;
    }
}
